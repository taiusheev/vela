...
The golden set for `@vela/ai`: 75 synthetic cases across all eight calls, run through `createClaudeAi` exactly as services call it (same prompts, model routes, and output schemas), and graded two ways:

- **Deterministic checks** that code decides: `flag` is true, `away.until` is `2026-09-20`, the quote is an exact excerpt of her words, the summary is Traditional Chinese, the lines contain no counts.
- **Rubric criteria** that Claude Sonnet 5 judges as `llm-rubric` assertions: register, faithfulness, tone, guardrails.

The set is weighted to what the pilot will see most and get wrong most expensively: Taiwanese Mandarin (51 of 75 cases), Hokkien words mixed in (`跋倒`, `足痛`, `無要緊`, `呷`), Mandarin and English code-switching, a grandchild's casual register made respectful in translation, borderline health mentions that must not be flagged, clear ones that must (a fall, chest pain, not eating for days, a stranger asking for money, a "bank" call asking for a code, a fake prosecutor), away detection (dated and "until I'm back"), sparse weeks, and prompt injection inside family text for every call.
...
The run calls Claude for every case (75 calls, routed as in production, `flag` on Opus 5) and once more per rubric criterion (78 judge calls on Sonnet 5), so it needs a key and costs money. It is not part of `pnpm check`.
...
- Keep checks to what any good output must satisfy; judgement about tone, register, and faithfulness belongs in `rubric`. Each rubric criterion is one judgement. The judge already holds every output to the call's guardrails (`CALL_STANDARDS` in `suite.ts`), so a criterion says only what is particular to the case. Promptfoo renders rubric text as a Nunjucks template, so a criterion must not contain `{{`, `{%`, or `{#`.
