...
A flag sends `flag.notice` with her words verbatim to each organiser (they are family), and `admin.flag` to the admin conversation when configured. The quote is the model's excerpt when it kept one that is exactly hers, and otherwise everything she said: the AI layer drops an excerpt that is not an exact substring of her words but keeps the flag, so a flag whose quote was dropped still reaches every organiser, with her whole message. `admin.flag` carries only `{family}` (the family name) and `{link}` (the family's admin page, `Config.publicBaseUrl` + `/admin/families/<family id>`), never her words: the admin conversation sits outside `admin_access_log` and retention, and opening the page is logged (§3.17). Both are `kind flag` (the budget's one exception), keyed by the exchange and the reader's conversation with the answer id as suffix. Event `flag_raised` (props `category`, `severity`, and `excerpt`: true when the quote is the model's excerpt; never the quote), recorded in the transaction that inserts the notices, so a re-run that inserts none records nothing.
...
- Every 5 minutes, `reconcile`, in this order:
  ...
  3. Active kept-light members of families without `deleted_at` whose `next_wake_at` is null or more than 10 minutes in the past are ticked; each late one logs the warning `scheduler_missed` and records the event `scheduler_missed` (`late_minutes`). A member whose tick throws is logged (`reconcile_tick_failed`, with `errorLabel`) and skipped.
  4. Answers not yet understood are re-run: those with `understood_at` null, `received_at` between 15 minutes and 24 hours ago, and `processing_attempts` below 3 are re-enqueued, a voice answer without a transcript to `ingest_answer_media` and any other to `understand_answer`. A re-run sends nothing twice (§3.10).
  5. The safety net for the founder's note (next item).
  6. The heartbeat is pinged at the end.
...
reconcile(deps): Promise<{ ticked: number; missed: number; rerun: number; effects: number }>   // pending send effects, stranded queued rows, late ticks, the understanding re-run, the founder's note, the heartbeat (§3.15)
// arrivals.ts
...
handleConsentButton(deps, event, action, tick = tickMember): Promise<void>
...
// called by the admin Worker with deps whose only working ports are db, clock, logger, queues.outbound, scheduler, and ai (AdminDeps, 03-code-design §9)
