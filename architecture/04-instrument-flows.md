...
On send, the gateway first commits the send result in its own transaction (`status sent`, `sent_at`, `external_id`, `message_refs`), then applies the kind's effects in a second transaction and stamps `outbound.effects_at`: the exchange moves to `delivered` (`delivered_at` = the send time, `delivery_late`), `read_back_at` is stamped on the included replies and the previous exchange moves to `read_back`, and her scheduler is woken. Effects are idempotent. A row already `sent` with `effects_at` null gets its effects applied and is never sent again, so a retry after a failed effect cannot produce a second message; `reconcile` sweeps such rows older than two minutes (event `outbound_effects_late`). Events `arrival_delivered`, `readback_delivered`.
