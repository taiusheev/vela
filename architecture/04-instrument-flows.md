...
The admin actions do not pass through `handleInbound`. The worker serves `/admin` behind Cloudflare Access: each `GET` of a page calls its read (`loadAdminOverview` then `loadFailedOutbound` for the overview, `loadFamilyPage` for a family), which writes the page's `view` rows before returning, and each action is a `POST` form that the worker accepts only with a valid Access JWT and a same-origin request, then calls the matching `admin.ts` function.
...
In `@vela/worker`: the webhook rejects a wrong secret with 401 and hands parsed events to the router; the Durable Object alarm calls the tick and re-arms at the returned time, and clearing the scheduler clears its storage; queue batches dispatch by job type and retry on thrown errors; cron calls reconcile, metrics, and retention, and a failed run, building its deps included, is logged as `cron_failed` by its label; `/admin` refuses a request without a valid Access JWT (401) and a `POST` from another origin (403) without calling services, and each page view calls its read with the identity in the token.
