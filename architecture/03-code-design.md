...
2026-09-16. How the code is organised, the rules every package follows, and the public API of each package. It turns `02-technical-architecture-v2.md` into buildable units and is the contract parallel work is built against. When code and this document disagree, fix one of them in the same change.
...
Modules and their entry points. `04-instrument-flows.md` §5 gives every signature; the names here are the same. The package root (`index.ts`) exports what the Worker wires: the admin reads and actions with their input and result types, `adminLink`, `ADMIN_OVERVIEW_PATH`, and `familyPagePath`; the port and job types; `VelaError` and `errorLabel`; `deliverOutbound` with `DeliveryResult`; `handleInbound`; `rollupMetrics` and `applyRetention`; `ingestAnswerMedia` and `understandAnswer`; and `tickMember` and `reconcile` with `ReconcileResult`. It also exports what the Worker does not use: `recordAdminView` with `AdminView` (each admin read writes its own views), `VELA_ERROR_CODES` with `VelaErrorCode`, and `draftWeeklyRead` and `loadScheduleInput`, which the tick calls itself. The test harness is `@vela/services/testing`.
...
- `src/index.ts` exports the Worker (`fetch`, `queue`, `scheduled`) and the `MemberScheduler` Durable Object class. Routes, the queue consumer, cron, and the Durable Object reach services through `WorkerRuntime` (`src/runtime.ts`), so tests hand them fakes; `productionRuntime` is the one place the real services are wired in.
- Routes (Hono): `GET /healthz` (builds no deps); `POST /webhooks/telegram` (verify the secret before deps are built, a wrong or missing one is 401; parse; handle; 200); the admin pages, server-rendered:
  ...
  - Every `/admin` request needs a valid Cloudflare Access token: the Worker verifies the `Cf-Access-Jwt-Assertion` header, an RS256 JWT, against the keys at `https://<ACCESS_TEAM_DOMAIN>/cdn-cgi/access/certs` (cached for an hour, fetched again for an unknown `kid`), its `aud` (holding `ACCESS_AUD`), `iss` (`https://<ACCESS_TEAM_DOMAIN>`), expiry, and not-before (60 seconds of skew), and takes the `email` claim as the admin identity. The Access application covers `/admin` only, so the webhook and `/healthz` stay reachable, and checking the token in the Worker as well means a mistake in the Access configuration cannot open the page. A request without a valid token gets 401. A POST must also be same-origin: its `Origin` header must be present and equal the origin of `PUBLIC_BASE_URL` (not whatever hostname the request arrived on), because the Access session cookie would otherwise let another site submit a form from the founder's browser; a POST that fails this gets 403. Neither refusal calls services, so neither changes nor logs anything. Only when `ENVIRONMENT` is `development`, where nothing sits in front of `wrangler dev`, a request with no token at all passes as the identity `development`.
- `MemberScheduler`: RPC `wakeAt(memberId, at)` stores the member id and sets the alarm no later than `at`: an alarm already set sooner stays, because the tick decides from the data, so an early wake costs one tick that finds nothing due while a later one could lose a wake asked for mid-tick. `wakeAt(memberId, null)` deletes the alarm and all stored state (a member who stopped, left, died, or was deleted keeps nothing in the object). `alarm()` calls `tickMember` with a scheduler port that sets this object's own alarm directly (an RPC to itself would wait on its own lock); a null result deletes the alarm, any other is armed no later than the returned instant. A tick that throws is logged as `scheduler_tick_failed` with `errorLabel` and tried again within a minute. An alarm that fires after the object was cleared does nothing.
- Queues: `vela-outbound` → `deliverOutbound`; `vela-media` → `ingestAnswerMedia`; `vela-understand` → `understandAnswer` (in staging and production the names end in `-staging` or `-production`). One set of deps per batch; a message is acked when its job returns and retried when it throws, logged as `queue_job_failed` with `errorLabel`, and after its `max_retries` it goes to the environment's one dead-letter queue (`vela-dead-letter`, `vela-dead-letter-staging`, `vela-dead-letter-production`). A body that is not a job is logged and acked.
- Cron: every 5 minutes (`*/5 * * * *`) `reconcile` (pending send effects, stranded sends, late ticks, the understanding re-run and the founder's note, then the heartbeat); nightly at 03:20 UTC `rollupMetrics`, then `applyRetention`. A run that throws, building its deps included, is logged as `cron_failed` with the cron and `errorLabel(error)`, and fails with an error whose message is only that label.
- Errors: an unexpected failure in a route answers 500 (so Telegram redelivers a webhook) and is logged as `request_failed` with the path, the method, and `errorLabel(error)`; every error the Worker logs (`request_failed`, `queue_job_failed`, `scheduler_tick_failed`, `cron_failed`) is logged by its label, never its message.
- Configuration: `wrangler.jsonc`, whose top level is local development (Worker `vela-dev`, run with `wrangler dev`) and whose `env.staging` and `env.production` are the Workers `vela-staging` and `vela-production`. Each deployed environment repeats its bindings with its own queue, dead-letter queue, R2 bucket (`vela-media-staging`, `vela-media-production`), and Hyperdrive id, and serves HTTP only on a custom-domain route whose host is that of its `PUBLIC_BASE_URL` (`workers_dev` is off; `src/wrangler-config.test.ts` holds the two together). The vars are `ENVIRONMENT`, `TELEGRAM_BOT_USERNAME`, `ADMIN_CONVERSATION_ID` (empty sends no admin messages, as in development), `PUBLIC_BASE_URL`, `PRIVACY_NOTICE_URL_EN`, `PRIVACY_NOTICE_URL_ZH_TW`, and `REGIONS`; the secrets, listed in `.dev.vars.example`, are `TELEGRAM_BOT_TOKEN`, `TELEGRAM_WEBHOOK_SECRET`, `ANTHROPIC_API_KEY`, `DEEPGRAM_API_KEY`, `HEALTHCHECKS_PING_URL`, `ACCESS_TEAM_DOMAIN`, and `ACCESS_AUD`. The local database is `@vela/db dev-db` reached through Hyperdrive's local connection string. `readConfig(env)` builds `Config`: `regions` from `REGIONS` (comma separated; the pilot: `apac`), `publicBaseUrl` from `PUBLIC_BASE_URL`, `privacyNoticeUrls` from `PRIVACY_NOTICE_URL_EN` and `PRIVACY_NOTICE_URL_ZH_TW` (other languages take the English URL), and `adminConversationId` from `ADMIN_CONVERSATION_ID`. There is no admin bearer token.
- Deployed configuration is checked before anything runs. Every host, privacy notice URL, bot username, chat id, and Hyperdrive id in staging and production is written as a `PLACEHOLDER_` value until the founder chooses it, never a guess (`vela.family` belongs to another company, `design/research-identity.md`). Outside development, `readConfig` throws `ConfigError` for the first string on the environment, var or secret alike, that contains `PLACEHOLDER_`, and for `PUBLIC_BASE_URL`, `PRIVACY_NOTICE_URL_EN`, or `PRIVACY_NOTICE_URL_ZH_TW` when it is not an https URL; a missing secret or required var throws it too. The error's `code` is the variable's name and its message never holds the value, so the failure is logged as, for example, `ConfigError:PUBLIC_BASE_URL`. `buildDeps` calls `readConfig` before it opens any connection, so a Worker deployed with a placeholder handles no webhook update, admin page, queue job, cron run, or alarm tick; `/healthz`, which builds no deps, still answers 200, so it does not prove the configuration.
- `compatibility_date` is pinned to the newest date the runtime of `@cloudflare/vitest-pool-workers` accepts (2026-08-22), and is raised together with that package, or `pnpm test` cannot start its runtime.
- Telegram setup: `pnpm --filter @vela/worker telegram:setup` runs `scripts/telegram-setup.ts` with `TELEGRAM_BOT_TOKEN`, `TELEGRAM_WEBHOOK_SECRET`, and `WORKER_URL` read from the environment. It refuses a secret outside `A–Z a–z 0–9 _ -` (1 to 256 characters) and a `WORKER_URL` that is not https, registers `<WORKER_URL>/webhooks/telegram` with `setWebhook` and `TELEGRAM_ALLOWED_UPDATES` imported from `@vela/adapters` (`message`, `callback_query`, `message_reaction`, `my_chat_member`), sets `/ask` and `/later` for group chats and no menu for private chats with `setMyCommands`, and prints the webhook URL, the updates, the bot's username, and whether group privacy is on, never the token or the secret.
- Deploy: `.github/workflows/deploy.yml` deploys through the GitHub environments `staging` (on a push to `main`, or a manual run the environment's rule limits to `main`) and `production` (on a `v*` tag, after the founder approves). Each job runs lint, typecheck, and the tests, then `pnpm --filter @vela/db migrate` against that environment's `DATABASE_URL`, then `wrangler deploy --env <environment>`, one run per environment at a time and never cancelled halfway. It reads three environment secrets in each, never repository secrets: `CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`, and `DATABASE_URL`. Until the founder has created an environment's secrets, its migrations are run by hand (`infra/runbooks/release.md`) (ADR-23).
- Tests with `@cloudflare/vitest-pool-workers` run inside workerd against the bindings in `wrangler.jsonc`, with fake secrets and fake services. They cover routing; webhook rejection; admin requests without a valid Access token (401) or from another origin (403) calling no service; the overview reading both lists with the token's identity; the typed confirmations and the forms' allowed values; escaping; the Access token checks; the Durable Object alarm loop, including a sooner wake asked for mid-tick being kept; queue dispatch and cron, including a failed run logged by its label; configuration refusals naming the variable and never its value; errors logged by label, never by message; and, for each deployed environment, a custom-domain route on the host of `PUBLIC_BASE_URL`, with every host and notice URL still a placeholder (the commit that sets the chosen hosts changes that test to name them).

## 10. Decisions taken during the build

Builders recorded the rows from Schedule to Copy during the foundation build. The rows from Media identity to Initial migration are the sprint 1 contract decisions of 2026-09-14, taken before services and the worker were built; the Time zones row was replaced by one of them. The rows from Outbound recovery down were taken while services and the worker were built (2026-09-15 and 16). All are part of the contract.
...
| Deploy | GitHub environments `staging` and `production`, where production requires the founder's approval. `CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`, and `DATABASE_URL` are environment secrets, never repository secrets. Migrations run in the deploy job (`.github/workflows/deploy.yml`) before `wrangler deploy`; until an environment's secrets exist, the founder runs them (build plan 0.3, ADR-23). |
...
| Deployed configuration | Every host, privacy notice URL, bot username, chat id, and Hyperdrive id in `wrangler.jsonc`'s staging and production is a `PLACEHOLDER_` value until the founder chooses it; a guess can belong to someone else (`vela.family` is another company's). Outside development the Worker refuses to run while any var or secret contains `PLACEHOLDER_`, anywhere in the value (`PUBLIC_BASE_URL` is `https://PLACEHOLDER_STAGING_HOST`), or while `PUBLIC_BASE_URL` or a privacy notice URL is not https: `readConfig` throws `ConfigError`, whose `code` is the variable's name, before any connection opens, and the failure is logged as `ConfigError:<variable>` (§9). Development runs on localhost and is not checked. Every caught error the Worker and services log is logged as `errorLabel(error)`, never its message (§2). |
