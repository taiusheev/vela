...
| `HEALTHCHECKS_PING_URL` | Secret | `vela` | Founder, at the setup script's prompt | Anyone holding it can fake the heartbeat |
...
| `TELEGRAM_BOT_USERNAME` | Variable | `vela` | Setup script | Per environment: that environment's bot username, without `@`, which the script asks Telegram for (`getMe`) and writes over the placeholder in `wrangler.jsonc`; the co-founder commits it (section 11) |
...
4. Copy each check's ping URL straight into the setup script's `Healthchecks ping URL` prompt for the matching environment (section 11), which puts it on the pilot Worker as `HEALTHCHECKS_PING_URL`.
...
6. **Run it.** For staging, then, once staging has passed, for production:
   ...
   | `telegram` | Checks the bot token with Telegram (`getMe`), writes the bot's username over `PLACEHOLDER_…_BOT_USERNAME` in `wrangler.jsonc`, reads your chat id (section 7, step 5), and generates the webhook secret (section 7, step 4). Skipped when the username is written and `vela` already holds the token, the webhook secret and your chat id |
   | `secrets` | Puts each secret on the Worker that reads it with `wrangler secret put`, the value on standard input: on `vela`, `TELEGRAM_BOT_TOKEN`, `TELEGRAM_WEBHOOK_SECRET`, `ADMIN_CONVERSATION_ID`, `ANTHROPIC_API_KEY`, `DEEPGRAM_API_KEY` and `HEALTHCHECKS_PING_URL`; on `vela-admin`, `ANTHROPIC_API_KEY`. A secret a Worker already holds is kept and not asked for, except the three from the `telegram` step, which are put whenever that step ran. The first put creates a Worker that does not exist yet, which the next step replaces |
   ...
   | `check` | Checks that `/healthz`, `/privacy` and `/privacy/zh-TW` on the pilot origin answer 200, and that `/admin` on the admin origin does not answer 200 without a sign-in (saying whether Cloudflare Access asked for one); then prints what to do next |
...
7. **The prompts, in order.** A prompt marked hidden shows nothing as you type or paste. One marked hidden until checked also shows nothing, then prints the identifier once it passes its check, so a key pasted there by mistake is never shown. Press Enter after each. A prompt is skipped when what it feeds already exists (step 6).
   ...
   | `secrets` | `Healthchecks ping URL (hidden)` | healthchecks.io, project "Vela", the check `vela-staging-reconcile` or `vela-production-reconcile` (period 5 minutes, grace 5 minutes; section 6), its ping URL | `HEALTHCHECKS_PING_URL` on `vela` |
