...
21 September 2026 · architecture §1 constraint 5, §14, §16 · code design §8 · build plan 1.3, 1.8, 3.6 and "the silence drill runs nightly from sprint 1"
...
**D · AI down (part 4).** Choose the path matching staging's `AI_PROVIDER`. This is deliberate failure injection on the staging pilot Worker `vela` only: the founder agrees to the specific change before it is made, has a working replacement key ready in the password manager, and restores service before leaving the drill. Never share a key in the drill log or a chat.

**While AI is off (the dogfooding week):** changing `ANTHROPIC_API_KEY` tests nothing, because neither Worker reads it. Instead, the founder temporarily replaces staging `vela`'s `DEEPGRAM_API_KEY` with a nonempty invalid value, using the hidden prompt or dashboard in [`secrets-rotation.md`](secrets-rotation.md). After a delivered arrival, the test member sends a scripted voice answer. Expect the light to be lit and the voice note to reach the family group, but no transcript; a failed speech-to-text `ai_calls` row; `understood_at` empty; and no quiet notice. Restore the working Deepgram key within 10 minutes of the answer. Between 15 and 30 minutes after the answer, reconciliation should retry ingestion and the transcript should appear exactly once; understanding then completes with AI off, without a summary, translation, flag notice, or Anthropic call. Staging's media storage is `r2` since 26 September 2026, so the first attempt already copies the voice into `vela-media-staging` and the retry reads it from there rather than from Telegram (with storage off, ingestion would fetch it again from Telegram and write no R2 object). If all three attempts fail first, expect one `admin.understand_failed` note and use a new voice answer after restoring the key to prove transcription works. Record the observed times and which provider was tested.

**Once AI is on:** the founder replaces the staging `ANTHROPIC_API_KEY` with an invalid value. The test member answers with a voice note.
