...
3. **Find the failing layer.**
   ...
   | Postgres unreachable | neonstatus.com; Neon plan limits: on the Free plan, each environment's project (`vela` for production, `vela-staging` for staging) has its own 100 compute hours a month, and when a project has used them up Neon suspends its compute until the next billing month (Neon console → that project → **Usage**; `infra/README.md`, section 2), so staging running out never stops production; storage; the Hyperdrive configuration, which both Workers bind. Telegram's webhook answers 500 and Telegram retries; the admin page fails too. Once LINE is on, LINE's webhook still answers 200 (it needs no database), so LINE redelivers nothing: its events wait in `vela-inbound-<environment>`, retried 30 seconds after their first attempt, doubling to every 10 minutes, for about 10 hours (`architecture/05-line-flows.md` §5.10), and each failed attempt is logged as `queue_job_failed` or `queue_deps_failed`. An answer given during an outage shorter than that is recorded within 10 minutes of the database coming back; after a longer one, see Dead-letter growth. On either channel, if her quiet deadline passed during the outage, the quiet notice and her waiting answer race once it ends, and an answer is recorded at the time it is handled, not the time she sent it: a quiet event resolved `answered_late` within minutes of the end is most likely ours, so send that organiser the "Answer held up" message below. A suspended `vela` needs the paid plan today, not next month: arrivals and quiet notices stop |
   ...
   | Dead-letter growth | The DLQ messages. Those of the outbound, media and understanding queues carry ids only: fix the cause, then re-drive. Once LINE is on, a `handle_inbound` message from `vela-inbound-<environment>` carries one webhook's events whole: her words, LINE user and group ids, reply tokens. It got there only after about 10 hours of retries, or because no retry could handle it, and nothing else holds those events: LINE was answered 200 and redelivers nothing, and `reconcile` has no row to re-drive. Fix the cause, then re-send each such body unchanged to `vela-inbound-<environment>` (Cloudflare dashboard, Queues) and remove it from the dead-letter queue. The handlers are idempotent, so a re-sent job changes nothing twice, and her answer in it still resolves an open quiet event. Re-send a job only while it is still the day its events were sent (each event's `at`) in her time zone and her next arrival has not gone out: an answer is recorded at the time it is handled, so later it would count as the next day's answer. Remove such a job unsent instead. Never copy a body into the incident note or anywhere else, as with query results (`data-requests.md`, rule 6). If her answer was in one and a quiet notice went out, send that organiser the "Answer held up" message below |
...
| Answer held up | [Name] did answer today. A problem on our side held the answer up, so today's quiet notice about [Name] was wrong. I'm sorry for the worry. Timur, Vela | [名字] 今天其實有回覆。我們這邊出了問題，回覆晚了才送到，所以今天關於 [名字] 沒有回覆的通知是錯的。讓您擔心了，真的很抱歉。Timur，Vela |
