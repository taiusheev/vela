...
Not secrets, no rotation needed: `SENTRY_DSN` (regenerate the client key only if it is abused), bot usernames, account and project ids, Hyperdrive ids, `ACCESS_TEAM_DOMAIN` and `ACCESS_AUD`. `ADMIN_CONVERSATION_ID` is stored as a secret of `vela` so the founder's personal chat id stays out of the repository, but nobody can message that chat without the bot token.
