...
18 September 2026 · architecture §13 · `infra/README.md` (rule zero) · done by the founder; the co-founder never sees a value
...
| `CLERK_SECRET_KEY` | Secret of `vela` in staging, while its `API_V1` is `on` (production's is `off` and holds none, ADR-29); the developer's copy in `apps/worker/.env.local`, for `api:dev` (`infra/README.md`, section 9a). Both are the one key of Clerk's development instance `ideal-vulture-9262` | Clerk → **Vela Light → Development → API keys → Secret keys** → roll; the old key stops at once. Then replace it in `apps/worker/.env.local` first, and put it on staging from that file by piping it into `pnpm exec wrangler secret put CLERK_SECRET_KEY --env staging` in `apps/worker` (`infra/README.md` §9a step 9 has the PowerShell lines; pasting into wrangler's hidden prompt on Windows PowerShell 5.1 stored a garbled key on 25 September 2026), or principle 5's dashboard path | Between the roll and the put, every write to staging's API answers 503 `unavailable`, because Clerk refuses the old key at the live session check; reads, sign-in, the Telegram pilot and families are unaffected | `https://vela.vela-light-staging.workers.dev/v1/me` without a token answers 401 (a 503 means the Worker refused the value put, for example one with a space in it, and the log line `api_config_refused` names `ConfigError:CLERK_SECRET_KEY`); one write from the app on staging, such as composing an ask, succeeds, which shows Clerk accepts the new key |
...
| Neon database password | Each environment's own Neon project (`vela-staging` or `vela`, role `neondb_owner`): its Hyperdrive configuration; GitHub `DATABASE_URL` in environment `staging` or in environment `production` (ADR-23) | Keep the role and reset its password: Neon → that project → Roles → `neondb_owner` → reset password (the old one fails at once). Copy the new direct string from **Connect** (`infra/README.md`, section 2, step 5) into that account's Hyperdrive configuration (**Storage & Databases → Hyperdrive →** `vela-apac-staging` or `vela-apac` → edit connection) and into GitHub `DATABASE_URL` of the matching environment. There is no zero-downtime path with a second role in the pilot: `neondb_owner` owns the database `neondb` and every table the migrations created, Neon's documentation says a role that owns a database cannot be deleted before the database is (neon.com/docs/manage/roles, checked 18 September 2026), and a new role would not own the tables, so the next migration would fail | A reset causes a few minutes of 500s from the pilot Worker's webhook (Telegram redelivers) and from the admin page, until Hyperdrive has the new password (principle 4) | Hyperdrive shows the configuration updated; the admin page loads (`/healthz` builds nothing); the next CI migration job connects |
