...
- **Where.** Every statement runs in the Neon console → project `vela-staging` → **SQL Editor**, the project's default branch, database `neondb`, never in `vela`. Step 1 deletes from **Cloudflare ("Vela staging" account) → R2 object storage → `vela-media-staging`** instead: staging runs with `MEDIA_STORAGE` `r2` since 26 September 2026 (`infra/README.md`, section 1, step 3), so the voice notes and photos of the week are copied there, and its query lists their keys. Step 6's chat is your chat with the staging bot.
