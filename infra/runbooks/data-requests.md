...
- **Where.** Every statement runs in the Neon console → project `vela-staging` → **SQL Editor**, the project's default branch, database `neondb`, never in `vela`. Step 1 has nothing to delete: staging runs with `MEDIA_STORAGE` `off` (decision M, 20 September 2026, `infra/README.md`, section 1, step 3), so no bucket exists in the "Vela staging" account, no `media` row has a storage key, and the query returns no row; the friend's voice notes and photos stay in Telegram's own chats, which Vela never copied. Step 6's chat is your chat with the staging bot.
