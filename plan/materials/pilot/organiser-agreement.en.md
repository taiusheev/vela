> **Draft for the pilot, not legal advice. To be reviewed by counsel before any public launch.**

# Vela pilot: organiser agreement

Version `organiser-agreement.v1` · 13 September 2026 · goes with `privacy-notice.v1`

This is what you, the organiser, and [FOUNDER FULL NAME] (the founder, who runs the pilot) agree to while your family takes part in the Vela pilot. It is written to be read in five minutes. Please read the privacy notice as well.

**Your family:** [family code] · **The person the light is for:** [Name] · **Organiser(s):** [names] · **Pilot starts:** [date]

## 1. What the pilot is

Vela is being tested with a small number of families. Each morning someone in your family asks [Name] something; [Name] answers with a tap or a voice message; the family hears the answer and replies; [Name] hears the replies the next morning. When [Name] answers, the family knows [Name] is fine. The pilot runs on Telegram, and on LINE for families in Taiwan once it is ready. We measure whether this daily exchange works, so the founder will ask you a few short questions along the way.

Vela is early software under test. Messages can be late, and some features will change during the pilot. We will tell you when something important changes.

## 2. What Vela does, and what it never does

- **Vela never contacts anyone automatically.** Every message to a nearby contact is sent by a person in your family. In this pilot, if a day stays quiet, you call the nearby contact yourself; Vela only shows you their names and numbers.
- **Vela is not a medical or emergency service.** It cannot tell whether [Name] is in danger and cannot send help. A quiet day is information for you, not an alarm. If you think [Name] needs help now, act as you would without Vela: call [Name], call a nearby contact, call emergency services.
- If [Name] has not answered about two and a half hours after the morning message, Vela sends it once more. If there is still no answer later, you receive a short note with the facts. For the first 14 days, while Vela learns [Name]'s usual time, that note waits until about eight hours after the morning message.
- If an answer mentions something like a fall, pain, a stranger at the door or a request for money, you see [Name]'s own words that day. Vela does not diagnose.
- Vela never tells [Name] that anyone worried, never shows [Name] missed days, and never asks [Name] why.
- If Vela fails to deliver the morning message, you are told once ("We couldn't reach [Name] on [channel] today"), and no quiet-day note is sent for that day.

## 3. What you agree to do

1. **Ask [Name] first.** Talk to [Name] about Vela before the founder's call. Nothing is switched on unless [Name] says yes to the founder and taps yes in the chat. You cannot say yes on [Name]'s behalf.
2. **Respect "stop".** If [Name] says stop, everything pauses. Please don't press [Name] to start again.
3. **Set up the family group with care.** Create a new Telegram group for Vela, with family members only. Share the privacy notice with everyone you add. Children under 13 do not join; a parent sends their questions for them. Keep other family conversations in your usual chat.
4. **Only name nearby contacts who agree.** Send them the consent message yourself (`nearby-contact-consent.en.md`) and pass their reply to the founder. Tell the founder if a contact asks to be removed.
5. **Tell the founder about changes that matter,** within a day if you can: [Name] goes into hospital, moves, changes phone or messaging account, travels (so Vela can pause the notes), or passes away. When anyone marks that [Name] has died, every scheduled message stops and nothing automated is sent again.
6. **Keep the family's words in the family.** Don't share [Name]'s answers outside the family without [Name]'s agreement.
7. **Help us learn, if you are willing.** Short calls with the founder at around day 14 and day 30, and a few questions at weeks 4 and 12. Every question is optional.

## 4. What the founder agrees to do

1. Run the pilot carefully, and answer your messages within one day.
2. Read your family's exchanges only to run the pilot and write your Sunday weekly read, and log each time your family's records are opened.
3. Tell you the same day if something on our side affected your family (a late or missing morning message, a message that should not have been sent), and within 72 hours if personal information may have been exposed.
4. Never sell your family's information, show advertising, or use anyone's words or voice to train AI models; never use a real answer to test Vela's AI without that person's separate yes.
5. Delete information as the privacy notice says, and act on any request to see, correct or delete it.
6. Tell you before anything in this agreement or the privacy notice changes.

## 5. The pilot fee

- The fee is **US$15** for the first month of the pilot, paid by a method you and the founder both already use (for example Wise, Revolut or PayPal). The founder confirms receipt.
- **It is refundable in full.** If Vela is not useful to your family, tell the founder at any time during that month or within 14 days after it. No reason is needed. The founder returns the full amount by the same method within 7 days and pays any transfer fee.
- If the founder ends or pauses the pilot for your family during the first month, the fee is refunded in full without your asking.
- Nothing renews automatically. If the pilot continues after the first month, the founder will ask you before any further payment, and you are never charged again without a new, separate yes.
- The prices of the future paid plan do not apply to the pilot.

## 6. Stopping

- You can end your family's pilot at any time by telling the founder. [Name] can stop at any time by saying "stop".
- The founder may pause the pilot for your family if something is not working safely, and will tell you why the same day.

## 7. When the pilot ends

If your family does not continue, the founder deletes your family's information within 30 days, as the privacy notice describes. Before that, the founder offers to send you any stories the family chose to keep. Messages in your Telegram or LINE chats stay there until you delete them.

## 8. Responsibility

Vela is an early test, provided as it is. Nothing in Vela replaces your own contact with [Name], a doctor, or emergency services. As far as the law allows, the founder's financial responsibility to you for the pilot is limited to refunding the pilot fee. Nothing in this agreement limits rights you have under consumer or data protection law. *[Counsel to review this section against Taiwan's Consumer Protection Act and the law where each family lives.]*

## 9. Agreeing

Reply **"I agree"** to the founder's message that contains this agreement, or say it on the onboarding call. The founder records your agreement (kind `pilot`, text version `organiser-agreement.v1`, the channel, the date, your words) and your receipt of the privacy notice (kind `privacy_notice`). Every other organiser in your family agrees the same way.

**Contact the founder:** [CONTACT ADDRESS]
