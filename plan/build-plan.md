...
2026-09-13. Sprint-by-sprint, task-level. Built from `architecture/02-technical-architecture-v2.md`, `product/05-product-spec-v2.md`, `plan/market-order.md`, and `plan/pre-build-readiness.md`. Twelve weeks in six sprints; each sprint has a definition of done that a non-technical founder can verify. The co-founder builds; the founder runs accounts, families, and reviews.
...
| 0.2 | Accounts: Cloudflare, Neon (project `vela-apac` in Singapore), Anthropic key, Deepgram key, Sentry, Healthchecks.io, Expo/EAS, Telegram bot (BotFather), LINE Official Account + Messaging API channel (unverified, founder's name) | **F** | Keys pasted into Worker secrets by the co-founder; `wrangler deploy` to staging succeeds |
| 0.3 | Schema applied via the first Drizzle migration to the apac branch; pglite test in CI | co-founder | 32 tables; the one-per-day and actor tests pass in CI |
...
| 1.3 | Member Durable Object: alarms for arrival, repeat, quiet, turn prompt; recompute from zone; reconciliation cron; heartbeat ping | arch §6 | Silence drill part 2 passes; Healthchecks shows a live check |
| 1.4 | Outbound gateway with the `outbound` table, budget index, actor CHECK, retries, DLQ | arch §7 | A second arrival for the same day is rejected in an integration test |
...
| 1.6 | Family Telegram group flow: 19:00 prompt, "reply to ask", `whenever:` items, "Into her morning" confirmation; her private-chat arrival with buttons; answer capture (light lit synchronously); read-back next morning | App. A | Founder's family completes one full loop: ask → answer → replies → read back |
...
| 1.8 | Quiet notice to the organiser in Telegram (facts, nearby contacts as text, buttons "she's fine", "wait 2 h"); learning period logic | §8 | A simulated silent day produces one notice at T_quiet, none on delivery failure |
| 1.9 | Event log + `metrics_daily` job + a first admin page (families, today's deliveries, answers, quiet events, AI outputs) | §18, arch §15 | Founder can see every family's day on one page |
...
| 2.7 | Weekly read v0 (AI draft, founder edits in the admin, sent by hand) | §13 | First reads sent Sunday of week 4 |
| 2.8 | Retention jobs (media, answers, members) and the deletion proof; restore drill #1 | §17 | A test family's media disappears after the window; `deletions` rows written; restore drill documented |
