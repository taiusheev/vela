...
2026-09-17. Sprint-by-sprint, task-level. Built from `architecture/02-technical-architecture-v2.md`, `product/05-product-spec-v2.md`, `plan/market-order.md`, and `plan/pre-build-readiness.md`. Twelve weeks in six sprints; each sprint has a definition of done that a non-technical founder can verify. The co-founder builds; the founder runs accounts, families, and reviews.
...
| 1.6 | **Built** Family Telegram group flow: 19:00 prompt, "reply to ask", `whenever:` items, "Into her morning" confirmation; her private-chat arrival with buttons; answer capture (light lit synchronously), counting a message sent before the arrival as that day's answer; her messages ignored until she taps yes and after a no; read-back next morning | App. A | Founder's family completes one full loop: ask → answer → replies → read back; a message typed before the consent tap reaches nobody |
...
Status on 17 September 2026: the code tasks marked **Built** are in the repository with their tests green, and nothing is deployed yet. They include sprint 2's retention job (2.8), built ahead of its sprint because the privacy notice's 30-day promise depends on it. 1.1's documents are drafts in `plan/materials/pilot/`, and the code records consents with their text version. The pilot is free for families (decided 17 September 2026): no fee is taken and no payment data is collected, and willingness to pay is asked on each organiser's day-30 call (6.2). Hosting was decided on 17 September 2026 (ADR-26): no domain is bought; each Cloudflare account's free workers.dev subdomain carries two Workers, the pilot Worker `vela` and the admin Worker `vela-admin`, and Cloudflare Access protects the whole admin Worker instead of a path on a domain (1.16). What remains before the pilot runs:

1. **F** Accounts and choices (0.2): the accounts in `infra/README.md`, sections 1 to 10, including each Cloudflare account's workers.dev subdomain, `vela-light` for "Vela" and `vela-light-staging` for "Vela staging" (section 1, step 10), which fixes the hosts already written in the wrangler files; each bot's BotFather settings, with the privacy policy link to its environment's `/privacy` page (section 7, step 3); and each GitHub environment's three secrets (section 10). Nothing else is handed over: the founder pastes each key at the setup script's hidden prompts (items 3 and 4). Both privacy notices were filled in on 17 September 2026 (the founder's name, contact address, and notes tool), because the pilot Worker serves them and refuses to start while one holds a bracketed blank.
2. Placeholders replaced: the `PLACEHOLDER_` values left for staging, then for production, are the bot usernames in `apps/worker/wrangler.jsonc` and the Hyperdrive ids in both `wrangler.jsonc` and `wrangler.admin.jsonc` (the hosts and notice URLs are already filled). The setup script writes each from what Telegram and Cloudflare return, and the co-founder commits them (`infra/README.md`, section 11, step 8); nobody hands them over. The script also reads the founder's chat id from Telegram and puts it straight into the pilot Worker's secret `ADMIN_CONVERSATION_ID`. The notices were filled and regenerated (`pnpm --filter @vela/worker notices`) on 17 September 2026. Both Workers refuse to run while a placeholder is left.
3. **F** Staging: the founder runs `pnpm --filter @vela/worker run setup -- --env staging` in their own terminal (`infra/README.md`, sections 11 and 12). It creates the queues, the dead-letter queue, and the bucket; migrates the Neon `staging` branch (0.3); creates Hyperdrive `vela-apac-staging` with caching off and writes its id; writes the bot username and reads the chat id; puts each Worker's secrets; deploys `vela`, then `vela-admin`; waits while the founder has Cloudflare Access protect the whole `vela-admin` Worker, then puts its two Access secrets; registers the Telegram webhook on the pilot origin; and checks the result. Then the smoke test and the silence drill on staging (`infra/runbooks/release.md`, step 3; `infra/runbooks/silence-drill.md`). Later staging deploys go through CI on merge to `main` (`release.md`). Staging holds only synthetic data and the founder's own test accounts, never a real family (`infra/README.md`, "Environments").
4. **F** Production, all in the founder's hands, since the co-founder never signs in to the "Vela" account: once staging has passed, the founder runs `pnpm --filter @vela/worker run setup -- --env production` on the commit proven on staging, with a setup token for the "Vela" account that expires the next day and is deleted after the run (`infra/README.md`, section 11, steps 3 and 8). The script does item 3's steps in the "Vela" account and on the Neon `main` branch (Hyperdrive `vela-apac`), and its deploy is the one production deploy from a laptop (`infra/README.md`, access rules). Then the 30-minute watch (`release.md`, step 7). Every later production deploy goes through a `v*` tag the founder approves (`release.md`, steps 4 to 6).
5. **F** The founder's own family on production for a week, before any friend family (1.10; `plan/materials/pilot/README.md`, "Before first use beyond the founder's own family"). First, the founder decides on the changes `plan/materials/pilot/legal-memo.md` rates High, which it asks for before Vela runs for any family, the founder's own included; none is made yet (pilot README, "Review notes", "Open items from `legal-memo.md`"). Retention runs nightly in production, where the family's data is, and that is what keeps the notice's 30-day promise.
