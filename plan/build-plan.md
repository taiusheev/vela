...
Status on 17 September 2026: the code tasks marked **Built** are in the repository with their tests green, and nothing is deployed yet. They include sprint 2's retention job (2.8), built ahead of its sprint because the privacy notice's 30-day promise depends on it. 1.1's documents are drafts in `plan/materials/pilot/`, and the code records consents with their text version. Hosting was decided on 17 September 2026 (ADR-26): no domain is bought; each Cloudflare account's free workers.dev subdomain carries two Workers, the pilot Worker `vela` and the admin Worker `vela-admin`, and Cloudflare Access protects the whole admin Worker instead of a path on a domain (1.16). What remains before the pilot runs:

1. **F** Accounts and choices (0.2): the accounts in `infra/README.md`, sections 1 to 10, including each Cloudflare account's workers.dev subdomain, `vela-light` for "Vela" and `vela-light-staging` for "Vela staging" (section 1, step 10), which fixes the hosts already written in the wrangler files; both privacy notices filled in (`[FOUNDER FULL NAME]`, `[CONTACT ADDRESS]`, `[NOTES TOOL]`, `[NOTES TOOL LOCATION]`), because the pilot Worker serves them and refuses to start while one holds a bracketed blank; the bot usernames, the Hyperdrive ids, and each GitHub environment's three secrets. The founder's chat ids go straight into the pilot Worker's secret `ADMIN_CONVERSATION_ID`.
2. Placeholders replaced: the `PLACEHOLDER_` values left for staging, then for production, are the bot usernames in `apps/worker/wrangler.jsonc` and the Hyperdrive ids in both `wrangler.jsonc` and `wrangler.admin.jsonc` (the hosts and notice URLs are already filled). The notices are regenerated (`pnpm --filter @vela/worker notices`) in the commit that fills them. Both Workers refuse to run while a placeholder is left.
3. Staging deploy: the queues, the dead-letter queue, and the bucket created; the migration applied (0.3); both Workers deployed by the deploy job, `vela` then `vela-admin`, with their secrets, Cloudflare Access on the whole `vela-admin` Worker, and the Telegram webhook on the pilot origin (`infra/README.md`, sections 11 and 12); the smoke test and the silence drill on staging (`infra/runbooks/release.md`, `infra/runbooks/silence-drill.md`). Staging holds only synthetic data and the founder's own test accounts, never a real family (`infra/README.md`, "Environments").
4. **F** Production deploy, all in the founder's hands, since the co-founder never signs in to the "Vela" account: the production queues, dead-letter queue, and bucket created in the "Vela" account's dashboard (`infra/README.md`, section 1, step 5); Hyperdrive `vela-apac` on the Neon `main` branch, caching off (section 2, step 7); a `v*` tag on the commit proven on staging, which the founder approves so the deploy job migrates Neon `main` and deploys `vela` and `vela-admin` (`infra/runbooks/release.md`, steps 4 to 6); then each Worker's secrets, Cloudflare Access on the whole `vela-admin` Worker, the bot's privacy policy link to `https://vela.vela-light.workers.dev/privacy`, and the Telegram webhook on `https://vela.vela-light.workers.dev` in the "Vela" account (`infra/README.md`, sections 7, 11 and 12), and the 30-minute watch (`release.md`, step 7).
5. **F** The founder's own family on production for a week, before any friend family (1.10; `plan/materials/pilot/README.md`, "Before first use beyond the founder's own family"). Retention runs nightly in production, where the family's data is, and that is what keeps the notice's 30-day promise.
...
| 2.5 | STT benchmark: 30 consented pilot clips, kept in the private R2 bucket `vela-benchmark` ("Vela staging" account; data map row 23 and gap 19), through Deepgram, gpt-4o-transcribe, SenseVoice (Groq/Whisper as baseline); WER table in the admin | arch §9.2 | Default chosen on measured WER; ADR-14 updated |
