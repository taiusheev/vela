...
2026-09-16. Sprint-by-sprint, task-level. Built from `architecture/02-technical-architecture-v2.md`, `product/05-product-spec-v2.md`, `plan/market-order.md`, and `plan/pre-build-readiness.md`. Twelve weeks in six sprints; each sprint has a definition of done that a non-technical founder can verify. The co-founder builds; the founder runs accounts, families, and reviews.
...
| 0.2 | Accounts: Cloudflare, Neon (project `vela-apac` in Singapore), Anthropic key, Deepgram key, Sentry, Healthchecks.io, Expo/EAS, Telegram bot (BotFather), LINE Official Account + Messaging API channel (unverified, founder's name); GitHub environments `staging` and `production` (production with the founder as required reviewer) | **F** | Keys pasted by the founder into Worker secrets, and `CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`, and `DATABASE_URL` into each GitHub environment's secrets (no repository secrets); `wrangler deploy` to staging succeeds |
...
Status on 16 September 2026: the code tasks marked **Built** are in the repository with their tests green, and nothing is deployed yet. They include sprint 2's retention job (2.8), built ahead of its sprint because the privacy notice's 30-day promise depends on it. 1.1's documents are drafts in `plan/materials/pilot/`, and the code records consents with their text version. What remains before the pilot runs:

1. **F** Accounts and choices (0.2): the accounts in `infra/README.md`, sections 1 to 10; a domain Vela owns as a zone in each Cloudflare account (a second one for staging, since Cloudflare activates a domain in one account at a time) and the Worker's host in each (never `vela.family`, which belongs to another company); the privacy notice published on that domain in both languages; the bot usernames, the admin chat ids, the Hyperdrive ids, and each GitHub environment's three secrets.
2. Placeholders replaced: every `PLACEHOLDER_` value for staging, then for production, in `apps/worker/wrangler.jsonc` (hosts, notice URLs, bot usernames, admin chat ids, Hyperdrive ids), with `src/wrangler-config.test.ts` changed in the same commit. The Worker refuses to run while one is left.
3. Staging deploy: the queues, the dead-letter queue, and the bucket created; the migration applied (0.3); the Worker deployed by the deploy job, with its secrets, Cloudflare Access on `/admin`, and the Telegram webhook (`infra/README.md`, sections 11 and 12); the smoke test and the silence drill on staging (`infra/runbooks/release.md`, `infra/runbooks/silence-drill.md`). Staging holds only synthetic data and the founder's own test accounts, never a real family (`infra/README.md`, "Environments").
4. **F** Production deploy, all in the founder's hands, since the co-founder never signs in to the "Vela" account: the production queues, dead-letter queue, and bucket created in the "Vela" account's dashboard (`infra/README.md`, section 1, step 5); Hyperdrive `vela-apac` on the Neon `main` branch, caching off (section 2, step 7); a `v*` tag on the commit proven on staging, which the founder approves so the deploy job migrates Neon `main` and deploys `vela-production` (`infra/runbooks/release.md`, steps 4 to 6); then the Worker's secrets, Cloudflare Access on `/admin`, and the Telegram webhook in the "Vela" account (`infra/README.md`, sections 11 and 12), and the 30-minute watch (`release.md`, step 7).
