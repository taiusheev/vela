...
| 1.14 | Weekly read send: the founder edits the Sunday draft on the admin page and taps Send; the sent lines are stored and go to each organiser in Telegram, and a Send that reaches no organiser is refused rather than recorded as sent; "what does the family see" shows the latest sent read after the summaries, translated into the parent's language when the family's differs | §9, §13, App. A | The founder's family receives its first weekly read from the Send button; the parent's "what does the family see" includes it, in the parent's language |
